...
def anketa_skip(update, context):
    ...
    update.message.reply_text(f"""<b>Имя Фамилия</b> {context.user_data['anketa']['name']}
    <b>Оценка</b> {context.user_data['anketa']['rating']}""", reply_markup=main_keyboard(), parse_mode=ParseMode.HTML)
...
def anketa_comment(update, context):
    ...
    <b>Комментарий</b> {context.user_data['anketa']['comment']}""", reply_markup=main_keyboard(), parse_mode=ParseMode.HTML)
    return ConversationHandler.END
